...
**AI Pass account card (optional):** Render a separate card above the provider form. Copy is "Connect AI Pass" and explains that requests spend from the connected account's shared AI Pass wallet. Never render AI Pass as a provider dropdown option or show an API-key field for it. Once connected, populate its model select only from the authenticated live catalog; show active model and Disconnect. A deployment without the protected public client ID shows a non-interactive administrator prerequisite message.
